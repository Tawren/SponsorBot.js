...
module.exports.run = async (client, message, args) => {
    ...
    if(!message.mentions.members.first()){
        if(!args[0]){
            ...
            message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
                ...
                channel.send(new Discord.MessageEmbed() 
                    ...
                    .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec Twren Puber !
                    Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser Tawren Puber, si vous n'avez pas encore de token, adressez-vous à un helper pour en savoir plus.`)
                    .setColor(config.embedColor)
                )
    
                channel.send(new Discord.MessageEmbed() 
                    .setTitle("Utilisation de Tawren Puber")
                    .setDescription("__Voici les commandes que vous pouvez utiliser avec Tawren Puber:__")
        ...
        message.guild.channels.create(target.user.username, {type: "text", permissionOverwrites: [
            ...
            channel.send(new Discord.MessageEmbed() 
                .setTitle(`**Bievenue dans votre salon privé, ${target.user.username}**`)
                .setDescription(`Vous disposez maintenant de votre propre salon privé pour faire des publicités avec Tawren Puber !
                Rappelez-vous que vous devez avoir un ou plusieurs tokens de bot pour utiliser Tawren Puber, si vous n'avez pas encore de token, adressez-vous à un helper pour en savoir plus.`)
                .setColor(config.embedColor)
            )
